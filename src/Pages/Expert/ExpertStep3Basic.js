...
function ExpertStep3Basic({ location }) {
  ...
  // 태그 추가 Event
  const handleAddTag = () => {
    if (tagInput.length > 20 || tagInput.length === 0) return;

    // 한글,영어,숫자만 입력가능
    if (!CHECK_KOR_ENG_NUM.test(tagInput)) {
    ...
    // 중복체크
    const dupCnt = tags.reduce((cur, tag) => (tag === tagInput ? 1 : 0), 0);
    if (dupCnt > 0) {
      alert('중복된 태그가 존재합니다.');
      return;
    }
              ...
              <FormItemList>
                <FormItem>
                  <InputBox>
                    ...
                    ></InputText>
                    <span>
                      <em>{introductionInput.length}</em>/30
                    </span>
                  </InputBox>
                </FormItem>
              </FormItemList>
